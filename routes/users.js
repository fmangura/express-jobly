...
const { ensureLoggedIn, ensureAdmin, isAdminOrUser } = require("../middleware/auth");
 ...
 * 
 * Takes user username and job id params and inserts into application table.
 * 
 * Authorization required: login
 */
...
router.post("/:username/jobs/:id", isAdminOrUser, async function (req, res, next) {
  try {
    ...
    let applied = await User.apply(username, id);
